...
import { Lesson, QuizQuestion, AppContextType, Performance } from '../types';
...
const QuizLesson: React.FC<QuizLessonProps> = ({ lesson, onComplete }) => {
  ...
  useEffect(() => {
    ...
    // Fallback to all questions if filtering leaves too few options
    const questionPool = filteredQuestions.length >= 5 ? filteredQuestions : allQuestions;

    // Determine a random number of questions between 5 and 10
    const maxQuestions = Math.min(10, questionPool.length);
    const minQuestions = Math.min(5, questionPool.length);

    const numQuestionsToAsk = (minQuestions <= maxQuestions)
      ? Math.floor(Math.random() * (maxQuestions - minQuestions + 1)) + minQuestions
      : maxQuestions;
    
    const finalQuestions = [...questionPool].sort(() => Math.random() - 0.5).slice(0, numQuestionsToAsk);
  ...
  return (
    <div className="p-6">
      <h3 className="text-2xl font-bold text-slate-800 text-center mb-2">
        {lesson.title} ({activeQuestions.length} preguntas)
      </h3>
